...
public:
	...
	void LibVectorTest_ScalarMultOperator()
	{
		...
		result = vec * 0;
		MY_ASSERT_TRUE(result == LibVector<double>(0, 0, 0));
	...
	void LibVectorTest_ScalarDivOperator()
	{
		LibVector<double> vec(1, 2, 3);
		auto result = vec / 2;
		MY_ASSERT_VEC_EQ(result, LibVector<double>(0.5, 1, 1.5));
		...
		result = vec / 0.5;
		MY_ASSERT_VEC_EQ(result, LibVector<double>(2, 4, 6.1));
	}

	void RunAllTests() {
		RUN_TEST(LibPointTest_DistanceTo);
		RUN_TEST(LibPointTest_Operators);
		RUN_TEST(LibVectorTest_Parallel);
		RUN_TEST(LibVectorTest_IsZero);
		RUN_TEST(LibVectorTest_ScalarMultOperator);
		RUN_TEST(LibVectorTest_ScalarDivOperator);
	}
};
